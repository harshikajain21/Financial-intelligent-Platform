# agents/market_data_agent.py

import yfinance as yf
import pandas as pd
from typing import Optional
from agents.base_agent import BaseAgent, AgentResult, AgentError


class MarketDataAgent(BaseAgent):
    """
    Agent 1: Market Data Intelligence Agent

    Fetches real-time and historical price data + fundamentals
    from Yahoo Finance. No API key required for basic usage.

    Output score represents data quality/completeness (0 to 100).
    """

    def __init__(self):
        super().__init__(name="MarketDataAgent", max_retries=3)

    def execute(self, symbol: str, period: str = "6mo", **kwargs) -> AgentResult:
        """
        Fetch market data for a given stock symbol.

        Args:
            symbol : stock ticker e.g. 'AAPL'
            period : how much history to fetch
                     valid values: 1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y
        """
        self.logger.info(f"Fetching market data for {symbol} | period={period}")

        # --- Step 1: Download price history ---
        ticker = yf.Ticker(symbol)
        hist = ticker.history(period=period)

        if hist.empty:
            raise AgentError(f"No price data returned for {symbol}")

        # --- Step 2: Clean the price data ---
        hist = hist.reset_index()
        hist.columns = [c.lower() for c in hist.columns]  # normalize column names

        # Keep only what we need
        price_data = hist[["date", "open", "high", "low", "close", "volume"]].copy()
        price_data["date"] = price_data["date"].astype(str)

        # --- Step 3: Fetch company fundamentals ---
        info = ticker.info
        fundamentals = self._extract_fundamentals(info)

        # --- Step 4: Calculate latest snapshot ---
        latest = price_data.iloc[-1]
        snapshot = {
            "symbol"        : symbol,
            "date"          : str(latest["date"]),
            "open"          : round(float(latest["open"]), 4),
            "high"          : round(float(latest["high"]), 4),
            "low"           : round(float(latest["low"]), 4),
            "close"         : round(float(latest["close"]), 4),
            "volume"        : int(latest["volume"]),
        }

        # --- Step 5: Package everything ---
        output_data = {
            "snapshot"      : snapshot,
            "fundamentals"  : fundamentals,
            "price_history" : price_data.to_dict(orient="records"),
            "bars_fetched"  : len(price_data),
        }

        # Score = data completeness (how many fundamental fields did we get?)
        score = self._calculate_data_quality_score(fundamentals)

        self.logger.info(
            f"{symbol} | Close: {snapshot['close']} | "
            f"Bars: {len(price_data)} | Quality score: {score}"
        )

        return AgentResult(
            agent_name=self.name,
            success=True,
            data=output_data,
            score=score,
            metadata={"symbol": symbol, "period": period}
        )

    def _extract_fundamentals(self, info: dict) -> dict:
        """
        Safely extract fundamental data from Yahoo Finance info dict.
        Uses .get() everywhere so missing fields don't crash us.
        """
        return {
            "market_cap"        : info.get("marketCap"),
            "pe_ratio"          : info.get("trailingPE"),
            "forward_pe"        : info.get("forwardPE"),
            "eps"               : info.get("trailingEps"),
            "dividend_yield"    : info.get("dividendYield"),
            "beta"              : info.get("beta"),
            "52w_high"          : info.get("fiftyTwoWeekHigh"),
            "52w_low"           : info.get("fiftyTwoWeekLow"),
            "avg_volume"        : info.get("averageVolume"),
            "sector"            : info.get("sector"),
            "industry"          : info.get("industry"),
            "country"           : info.get("country"),
            "employees"         : info.get("fullTimeEmployees"),
            "revenue"           : info.get("totalRevenue"),
            "profit_margin"     : info.get("profitMargins"),
            "roe"               : info.get("returnOnEquity"),
            "debt_to_equity"    : info.get("debtToEquity"),
            "current_ratio"     : info.get("currentRatio"),
            "short_name"        : info.get("shortName"),
            "exchange"          : info.get("exchange"),
        }

    def _calculate_data_quality_score(self, fundamentals: dict) -> float:
        """
        Score from 0-100 based on how many fundamental fields
        came back with actual data vs None.
        This tells downstream agents how much to trust this data.
        """
        total_fields = len(fundamentals)
        filled_fields = sum(1 for v in fundamentals.values() if v is not None)
        return round((filled_fields / total_fields) * 100, 2)

    def validate_output(self, result: AgentResult) -> bool:
        """
        Sanity checks before passing data forward:
        - Must have succeeded
        - Must have price history
        - Must have at least 1 bar of data
        """
        if not result.success:
            return False
        if not result.data:
            return False
        if result.data.get("bars_fetched", 0) == 0:
            return False
        return True